...
export const CountersView = () => {
    ...
    return (
        <div className="flex flex-col h-full bg-background overflow-y-auto overflow-x-hidden p-4 md:p-8">
            ...
            <div className="rounded-xl border border-border shadow-sm bg-card/50 mt-4 overflow-x-auto">
                    ...
                    <thead className="bg-secondary/50 sticky top-0 backdrop-blur-sm z-10">
                        <tr>
                            ...
                            {Object.keys(data[0] || {}).filter(k => k !== 'date').map(key => {
                                // Find label (v5.0.3 case-insensitive)
                                let label = key;
                        ...
                        {loading && data.length === 0 ? (
                            ...
                            data.map((row, i) => (
                                <tr key={row.date + i} className="hover:bg-muted/50 transition-colors">
                                    ...
                                    {Object.keys(data[0] || {}).filter(k => k !== 'date').map(key => {
                                        // Find color (v5.0.3 case-insensitive)
